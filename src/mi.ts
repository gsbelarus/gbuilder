...
async function _mi(params: IParams, log: Log) {
  ...
  const makeInstallation = (project: InstProject) => async () => {
    ...
    if (upload) {
      await uploadFile(arcFullFileName, 'https://gsbelarus.com/gs/content/upload.php');
      await uploadFile(portableArcFullFileName, 'https://gsbelarus.com/gs/content/upload.php');
      //await Promise.all([
      //  uploadFile(arcFullFileName, 'https://gsbelarus.com/gs/content/upload.php'),
      //  uploadFile(portableArcFullFileName, 'https://gsbelarus.com/gs/content/upload.php')
      //]);
    } else {
